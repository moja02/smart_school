...
<div class="card page-header-card mb-4 shadow border-0 bg-dark text-white" style="border-radius: 1rem;">
    <div class="card-body p-4 d-flex justify-content-between align-items-center">
        <div>
            <h2 class="fw-bold mb-1 text-white">جدول الامتحانات 📅</h2>
            <p class="text-white-50 mb-0">جدول الأيام القادمة ومواعيد امتحاناتك بدقة.</p>
        </div>
        <div>
            <a href="{{ route('student.dashboard') }}" class="btn btn-light text-dark rounded-pill px-4 fw-bold shadow-sm">
            ...
            @if($day['has_exam'])
                ...
                <div class="card h-100 shadow border-0 hover-scale border-top border-4 border-danger bg-white" style="border-radius: 1rem;">
                    <div class="card-body text-center p-3 d-flex flex-column justify-content-center">
                        {{-- اللون الأحمر هنا فقط كعلامة لليوم --}}
                        ...
                        {{-- طباعة أسماء المواد التي بها امتحانات في هذا اليوم --}}
                        @foreach($day['exams'] as $exam)
                            <a href="{{ route('student.subjects.show', $exam->subject_id) }}" class="text-decoration-none">
                                <span class="badge bg-danger rounded-pill w-100 py-2 mb-1 text-wrap shadow-sm">
                                    <i class="fas fa-exclamation-circle me-1 text-warning"></i> {{ $exam->subject->name }}
                                </span>
                            </a>
                        @endforeach
                    </div>
...
<style>
    /* تم تغيير لون ظل التحويم ليكون ظلاً عادياً بدلاً من الأحمر */
